...
sudo journalctl -u cysic-verifier -n 2 --no-hostname >> current_logs.txt
echo -e "\n" >> current_logs.txt

echo "Initverse" >> current_logs.txt
sudo journalctl -u initverse-miner -n 2 --no-hostname >> current_logs.txt 
echo -e "\n" >> current_logs.txt 

echo "Pipe Network"
./pop --status >> current_logs.txt
echo -e "\n" >> current_logs.sh
